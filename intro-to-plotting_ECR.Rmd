...
```{r}
#My code
#Set the director manually, then read in the data
penguins = read.table('C:/Users/Elizabeth Ross/Documents/PLB812/penguins.txt', header = T)
```
...
```{r}
body_mass_g = as.numeric(penguins$body_mass_g)
...
```{r}
#body_mass is already in the enviroment
ggplot(penguins, aes(x = body_mass_g, fill = species)) +
...
```{r}
bill_depth_mm = as.numeric(penguins$bill_depth_mm)
flipper_length_mm = as.numeric(penguins$flipper_length_mm)
